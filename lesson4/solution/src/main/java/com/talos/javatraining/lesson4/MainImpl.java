...
public class MainImpl implements Main
{
	...
	public AddressModel getBillingAddress(UserModel userModel)
	{
		return Optional.ofNullable(userModel)
				...
				.map(addresses ->
						getAddress(addresses, a -> BooleanUtils.isTrue(a.getBillingAddress()))
				)
				.orElse(null);
	}
	...
	public String getLastLoginFormatted(UserModel userModel)
	{
		return Optional.ofNullable(userModel)
				...
				.map(lastLogin -> {
					...
					return format.format(lastLogin);
				})
	...
	public String getContactCountry(UserModel userModel)
	{
		return Optional.ofNullable(userModel)
				.map(UserModel::getAddresses)
				.filter(CollectionUtils::isNotEmpty)
				.map(addresses -> getAddress(addresses, a -> BooleanUtils.isTrue(a.getContactAddress())))
	...
	public AddressModel getShippingAddress(UserModel userModel) throws AddressNotFoundException
	{
		return Optional.of(userModel)
				.map(UserModel::getAddresses)
				.filter(CollectionUtils::isNotEmpty)
				.map(addresses -> getAddress(addresses, a -> BooleanUtils.isTrue(a.getShippingAddress())))
