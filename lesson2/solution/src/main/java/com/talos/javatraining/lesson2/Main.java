...
public class Main
{
	...
	protected Measure<String> makeItReadable(Measure<BigDecimal> measure)
	{
		 ...
		 */
		manageData(measure::getWindSpeed, result::setWindSpeed, "mph");
		manageData(measure::getTemperature, result::setTemperature, "°C");
		manageData(measure::getPressure, result::setPressure, "pa");
		manageData(measure::getHumidity, result::setHumidity, "%");
		return result;
	}

	protected void manageData(Supplier<BigDecimal> source, Consumer<String> target, String unit)
	{
		target.accept(makeItReadable(source.get(), unit));
	}
